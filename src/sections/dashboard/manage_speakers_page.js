...
class Manage_speakers_page extends Handle_file_upload {
  constructor(props) {
    super(props);
  ...
  componentDidMount = async () => {
    let speakers = await get_request("speakers_page");
  ...
  submit = async () => {
    let { sections, image, image_file_hash, title, speakers } = this.state;
    if (!speakers) return;

    this.setState({ updating: true, message: null });

    speakers = { sections, image, image_file_hash, title };

    let res = await post_request("update_speakers", speakers);
...
export default Manage_speakers_page;
